...
// ---------------------------------------------------------
    ...
    fun getNestedSubTopicsFor(
        belt: Belt,
        topicTitle: String,
        subTopicTitle: String
    ): List<SubTopic> {
        val wanted = subTopicTitle.normHeb()

        return getSubTopicsFor(belt, topicTitle)
            .firstOrNull { it.title.normHeb() == wanted }
            ?.subTopics
            .orEmpty()
    }

    fun getNestedSubTopicTitles(
        belt: Belt,
        topicTitle: String,
        subTopicTitle: String
    ): List<String> =
    ...
    fun getNestedItemsFor(
        ...
        val wanted = nestedSubTopicTitle.normHeb()

        return getNestedSubTopicsFor(belt, topicTitle, subTopicTitle)
            .firstOrNull { it.title.normHeb() == wanted }
            ?.items
            .orEmpty()
    }
    ...
    fun getAllItemsFor(
        ...
        // אם ביקשו תת־נושא – התאמה מדויקת בלבד.
        // אם לתת־נושא יש רמה פנימית, מחזירים את כל התרגילים שבתוכה.
        subTopicTitle?.let { wanted ->
            val wn = wanted.normHeb()
            val matched = subs.firstOrNull { it.title.normHeb() == wn } ?: return emptyList()

            if (matched.subTopics.isNotEmpty()) {
                return matched.subTopics.flatMap { it.items }
            }
        ...
        // כולל תמיכה ברמה פנימית.
        return subs.flatMap { st ->
            if (st.subTopics.isNotEmpty()) {
                st.subTopics.flatMap { it.items }
            } else {
                st.items
            }
        }
    ...
    """.trimIndent()
                ...
                SubTopic(
                    ...
                    generalNote = """
        ההגנות נעשות בהתאם לאחיזת הסכין (לא כנגד הסכין).
        יש לבצע את התרגילים כנגד נשק ביד ימין וכנגד נשק ביד שמאל.
        התרגילים המתוארים - הם כנגד יד ימין.
    """.trimIndent()
                ),
                SubTopic(
                    "עבודת קרקע",
    ...
    private val greenBeltTopics = listOf(
        ...
        Topic(
            ...
            subTopics = listOf(
                ...
                SubTopic(
                    ...
                    generalNote = """
                        עם כיוון המשיכה או הסיבוב – הרמת יד להגנה והמשך התקפה למפשעה או לפנים, בהתאם למיקום התוקף.
                    """.trimIndent()
                ),
                SubTopic(
                    "שחרורים מחביקות צואר",
        ...
        Topic(
            ...
            generalNote = """
                יש לסיים את התרגילים עם השתלטות.
            """.trimIndent()
        ),
        Topic(
            "מכות במקל / רובה",
    ...
    private val brownBeltTopics = listOf(
        ...
        Topic(
            "שחרורים",
            listOf("חביקת צואר מאחור – בריח על העורף, המגן כפוף לפנים")
        ),
        Topic(
            title = "הגנות",
            items = emptyList(),
