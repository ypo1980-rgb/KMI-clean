 ...
 */
fun buildTopicsPickToken(selection: PracticeByTopicsSelection): String {
...
fun NavGraphBuilder.homeNavGraph(
            ...
            HomeScreen(
                ...
                onOpenExercise = { key: String ->
                    ...
                    if (item.startsWith("$topic::")) {
                        item = item.removePrefix("$topic::")
                    }

                    val display = ExerciseTitleFormatter.displayName(item)
                        .ifBlank { item }
                        .trim()
    ...
    composable(
        ...
        val itemTitle = remember(idEnc) {
            runCatching { java.net.URLDecoder.decode(idEnc, "UTF-8") }
                .getOrDefault(idEnc)
                .replace("+", " ")
                .trim()
        }
        ...
        BeltQuestionsByTopicScreen(
            ...
            onOpenTopicWithSub = { belt: Belt, topicTitle: String, subTopicTitle: String ->
                if (
                    ...
                    isLockedPremiumDefenseRoute(topicTitle, subTopicTitle)
                ) {
                    nav.navigate(Route.Subscription.route) {
                        launchSingleTop = true
                        restoreState = true
...
private fun resolveCanonicalItemTitle(
    ...
    if (cleaned.startsWith("$topicTitle::")) {
        cleaned = cleaned.removePrefix("$topicTitle::")
    }
