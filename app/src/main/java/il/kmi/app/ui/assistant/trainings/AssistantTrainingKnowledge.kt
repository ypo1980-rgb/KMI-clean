...
""".trim()
    ...
    fun buildTodayTraining(list: List<TrainingRow>): String {
...
object AssistantTrainingKnowledge {
    ...
    fun generateAnswer(question: String, memory: AssistantMemory): String {
        ...
        if (intent == AssistantIntent.UNKNOWN && lastIntent != null) {
            intent = AssistantIntent.valueOf(lastIntent)
        }
        ...
        if (results.isEmpty()) {
            ...
            if (askedToday && branch != null) {
                ...
                if (altAnyToday != null) {
                    return "אין היום אימון בסניף $branch, אבל יש ב-${altAnyToday.branchName} " +
                ...
                if (nextAny != null) {
                    return "אין היום אימון בסניף $branch. " +
        ...
        val answer = when {
            ...
            isTodayTrainingQuestion(norm) ->
                AnswerBuilder.buildTodayTraining(results)

            wantsUpcoming ->
                AnswerBuilder.buildUpcomingTrainings(results, branch, group, limit = 5)
...
private fun isTodayTrainingQuestion(norm: String): Boolean {
    return listOf(
        "יש לי אימון היום",
        "מתי האימון היום",
        "האימון שלי היום",
